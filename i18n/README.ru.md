...
- **🔎 Эмбеддинги** → семантический + кросс-языковой поиск. Модель превращает текст в векторы; они хранятся в *той же SQLite* и сливаются с BM25. (например, `all-minilm` 45 MB EN · `paraphrase-multilingual` ~560 MB мультиязычная)
- **🌱 Консолидация** → небольшая фоновая LLM, которая дедуплицирует/сливает память в фоне, безопасно через предложения. (например, `qwen2.5:1.5b` ~1 GB)
...
**Бонус:** извлечение переходит от «только по ключевым словам» к **гибридному (BM25 + плотные векторы, слитые вместе)** — оно находит перефразирования и кросс-языковые совпадения, а не только точные слова. recall@1 подскакивает **0.77 → 0.94** (recall@3 → 1.0 с мультиязычной моделью). Фоновая модель держит память в порядке. Всё остаётся **на 100% локальным — ноль API-токенов, никакого облака.**
